...
static char	*ft_dup_to_nl(char *saved_str)
{
	char	*output_line;
	size_t	nl_point;
	...
	if (saved_str[nl_point] == '\n')
		nl_point++;
	if (nl_point == ft_strlen(saved_str))
		return (saved_str);
	output_line = (char *)malloc (sizeof(char) * \
	(nl_point + 1 + (saved_str[nl_point] == '\n')));
	if (output_line == NULL)
		return (ft_free_str(saved_str));
...
static char	*dup_to_null(char *saved_str)
{
	size_t		nl_point;
	char		*tmp_str;
...
char	*get_next_line(int fd)
{
	...
	if (saved_str == NULL)
		return (NULL);
	output_line = ft_dup_to_nl(saved_str);
	if (output_line == NULL)
		return (NULL);
	tmp_str = dup_to_null(saved_str);
	if (output_line == NULL)
		return (NULL);
	ft_free_str(saved_str);
	saved_str = NULL;
